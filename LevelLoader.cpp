...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
        ...
        {
            LaserMine* mLaserMine = new LaserMine(game, parent);
            actor = mLaserMine;
        }
        else if(type == "Coin")
        {
            Coin* mCoin = new Coin(game, parent);
            actor = mCoin;
        }
        else if(type == "SecurityCamera")
        {
            SecurityCamera* mSCam = new SecurityCamera(game, parent);
            Quaternion startQ_;
            if (GetQuaternionFromJSON(actorValue, "startQ", startQ_))
            {
                mSCam->setStartQ(startQ_);
            }
            Quaternion endQ_;
            if (GetQuaternionFromJSON(actorValue, "endQ", endQ_))
            {
                mSCam->setEndQ(endQ_);
            }
            float interpTime_;
            if (GetFloatFromJSON(actorValue, "interpTime", interpTime_))
            {
                mSCam->setinterpTime(interpTime_);
            }
            float pauseTime_;
            if (GetFloatFromJSON(actorValue, "pauseTime", pauseTime_))
            {
                mSCam->setpauseTime(pauseTime_);
            }
            actor = mSCam;
        }
